...
import { AGENCY_MENU, filterMenusByPermission } from "@/lib/menus";
...
export default function AgencyDashboard() {
  ...
  const menus = filterMenusByPermission(AGENCY_MENU, account.permission);
    ...
    <div className="relative min-h-screen w-full overflow-hidden">
      ...
      <main className="relative max-w-6xl mx-auto px-4 sm:px-6 pt-28 pb-12">
        ...
        <p className="mt-1 text-white/80 drop-shadow">
          {account.role_display_name} · 에이전시 대시보드
        </p>
        ...
        <div className="mt-10 grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-8 sm:gap-10">
          {menus.map((m) => (
            <Link
              key={m.key}
              href={m.href}
              className="rounded-xl border border-white/15 bg-white/10 backdrop-blur-md p-8 min-h-[180px] grid grid-cols-[minmax(8rem,auto)_1fr] gap-8 items-center hover:bg-white/15 hover:border-white/30 hover:shadow-2xl transition-colors text-left"
            >
              <div>
                <m.Icon
