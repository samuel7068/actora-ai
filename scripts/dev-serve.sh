...
#
# 잠금을 이 시점까지 쥐고 있으려면 exec 를 쓸 수 없으므로 백그라운드로 띄우고,
# 시그널은 trap 으로 자식에게 넘긴다 (VS Code 가 태스크를 멈출 때 서버가 함께 죽어야 한다).
echo "· 기동: $*"
"$@" &
child=$!
...
for _ in $(seq $PORT_WAIT); do
  [ -n "$(listeners)" ] && break
  kill -0 "$child" 2>/dev/null || break   # 기동에 실패해 죽었다
  sleep 0.25
done
